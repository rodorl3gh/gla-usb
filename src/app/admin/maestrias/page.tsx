...
export default function MaestriasPage() {
  ...
  const load = useCallback(async () => {
    const token = localStorage.getItem("admin_token");
    const res = await fetch("/api/maestrias", { headers: { Authorization: `Bearer ${token}` } });
  ...
  async function toggleActive(s: any) {
    await api(`/api/maestrias/${s.id}`, "PUT", { activo: s.activo ? 0 : 1 });
    load();
  }

  async function remove(id: number) {
    if (!confirm("¿Eliminar esta maestría?")) return;
    await api(`/api/maestrias/${id}`, "DELETE");
  ...
  async function saveEdit() {
    ...
    await api(`/api/maestrias/${expandedId}`, "PUT", draft);
    setExpandedId(null);
    load();
  }
  ...
  async function create() {
    ...
    await api("/api/maestrias", "POST", newForm);
    setShowNew(false);
  ...
  return (
    <div style={{ animation: "page-in 0.35s ease-out" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.75rem", flexWrap: "wrap", gap: "1rem" }}>
        <div>
          <h1 style={{ fontSize: "1.5rem", fontWeight: 700, margin: 0, fontFamily: "var(--font-display)" }}>Maestrías</h1>
          <p style={{ margin: "0.25rem 0 0", fontSize: "0.85rem", color: "var(--admin-text-muted)" }}>
            {items.length} posgrados · haz clic en uno para editar su información
          </p>
        </div>
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.4rem 0.75rem" }}>
                ...
                >
                  ...
                  >
                    <BookOpen size={17} color={active ? "var(--admin-accent)" : "var(--admin-text-muted)"} />
                  </div>
              ...
              {expanded && (
                <div style={{ borderTop: "1px solid var(--admin-border)", padding: "1rem 1.25rem", display: "flex", flexDirection: "column", gap: "0.85rem" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: "0.75rem" }}>
                    ...
                    <label style={{ display: "flex", flexDirection: "column", gap: "0.35rem", fontSize: "0.75rem", fontWeight: 600, color: "var(--admin-text-secondary)" }}>
                      Duración
                      <input value={draft.duracion} onChange={(e) => setDraft({ ...draft, duracion: e.target.value })} placeholder="Ej. 2 años" style={inputStyle} />
                    </label>
                    <label style={{ display: "flex", flexDirection: "column", gap: "0.35rem", fontSize: "0.75rem", fontWeight: 600, color: "var(--admin-text-secondary)" }}>
                      RVOE
                      <input value={draft.rvoe} onChange={(e) => setDraft({ ...draft, rvoe: e.target.value })} placeholder="Ej. R.V.O.E NO 2009327" style={inputStyle} />
                    </label>
      ...
      {showNew && (
        <Modal title="Agregar maestría" onClose={() => setShowNew(false)}>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.35rem", fontSize: "0.75rem", fontWeight: 600, color: "var(--admin-text-secondary)" }}>
              Nombre
              <input value={newForm.nombre} onChange={(e) => setNewForm({ ...newForm, nombre: e.target.value })} style={inputStyle} placeholder="Ej. Derecho Fiscal" autoFocus />
            </label>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.35rem", fontSize: "0.75rem", fontWeight: 600, color: "var(--admin-text-secondary)" }}>
              Duración
              <input value={newForm.duracion} onChange={(e) => setNewForm({ ...newForm, duracion: e.target.value })} style={inputStyle} placeholder="Ej. 2 años" />
            </label>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.35rem", fontSize: "0.75rem", fontWeight: 600, color: "var(--admin-text-secondary)" }}>
              RVOE
              <input value={newForm.rvoe} onChange={(e) => setNewForm({ ...newForm, rvoe: e.target.value })} style={inputStyle} placeholder="Ej. R.V.O.E NO 2009327" />
            </label>
            <label style={{ display: "flex", flexDirection: "column", gap: "0.35rem", fontSize: "0.75rem", fontWeight: 600, color: "var(--admin-text-secondary)" }}>
              Descripción
            ...
            <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.25rem" }}>
              <button onClick={create} className="btn-primary" style={{ padding: "0.55rem 1.5rem", fontSize: "0.85rem" }}>Guardar maestría</button>
